...
def player_input(sw: str)->tuple:
    '''
    select the shape of list for each player 
    
    input:
        sw(start with)(nobat)(player1 or player2)
    ...
    while True:   
        piece1 = input(f'{sw} choose X or o: ')
        if piece1 in ['X', 'o']:
            break
...
while True:
    ...
    markers = player_input(start_with)
    while True: 
        marker = markers[0] if turn==start_with else markers[1]
